...
app = Flask(__name__) #session key
...
def load_logs():     # checks to see if logs.json exists.
    if os.path.exists(LOG_FILE):
        with open(LOG_FILE, "r") as file:    # opens and tries to load data.
            try:
...
def load_cpu_data(): # similair to the above. 
...
# Save logs to file
def save_logs():  #saves the logs once the flask closes.
    with open(LOG_FILE, "w") as file:
        json.dump(logs, file, indent=4)

# Save CPU data to file
def save_cpu_data():    #saves the logs once the flask closees.
...
def check_user_credentials(username, password):  
    conn = sqlite3.connect("users.db")  # This opens a connection to the SQllite SDBs. (users.db)
    cursor = conn.cursor() #used to excute queries! Without this, we wouldnt be able to log in.
    
    # Check if the username and password match
    cursor.execute("SELECT * FROM users WHERE username = ? AND password = ?", (username, password)) #Used to check for usernames/Passwirds
    user = cursor.fetchone() #returns if there is a legitimate recordf in the DB.
    conn.close() #closes connection to DB. 
    
    return user #retuns the user.

@app.route('/')
def dashboard():
    """Serves the main HTML dashboard."""
    if 'logged_in' not in session: # session key
        return redirect(url_for('login'))  # Redirect to login if not logged in!! This is used so ppl cant skip the login! 
    return render_template('dashboard.html') #returns the template once logeed in.
...
def sql_vulnerable_login():
    ...
    if request.method == 'POST':
        ...
        conn = sqlite3.connect("ctf.db")  #estabkushed connection to DB.
        cursor = conn.cursor() 
        ...
        try:
            ...
            # So the stupid ewrror at the ewnd goes away!!!!
            user = None
        finally:
            conn.close()

        if user:
            return render_template('ctf.html', message=f"Login Successful! Welcome, guehgereoghge| {username}")
        else:
